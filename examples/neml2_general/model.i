# NEML2 mixed strain/stress control model: Voce isotropic hardening with
# temperature-interpolated R(T) and d(T), and Kocks-Mecking flow with a
# temperature-interpolated shear modulus mu(T).

[Tensors]
  [T_controls]
    type = Python
    expr = 'Scalar(torch.tensor([300, 347.36842105, 394.73684211, 442.10526316, 489.47368421, 536.84210526, 584.21052632, 631.57894737, 678.94736842, 726.31578947, 773.68421053, 821.05263158, 868.42105263, 915.78947368, 963.15789474, 1010.52631579, 1057.89473684, 1105.26315789, 1152.63157895, 1200], dtype=torch.float64), sub_batch_ndim=1)'
  []
  [mu_values]
    type = Python
    expr = 'Scalar(torch.tensor([76670.48346056, 75465.18012589, 74314.80514263, 73374.72880675, 72651.54680595, 71928.36480514, 71120.75130575, 70035.97830454, 68951.20530333, 67842.26597027, 66399.97991161, 65315.20691041, 63884.85335476, 62763.98151868, 61373.80474086, 59927.44073925, 58481.07673765, 56544.43551627, 54599.93973483, 52791.98473282], dtype=torch.float64), sub_batch_ndim=1)'
  []
  [T_train]
    type = Python
    expr = 'Scalar(torch.tensor([300.0, 600.0, 900.0, 1200.0], dtype=torch.float64), sub_batch_ndim=1)'
  []
  [R_values]
    type = Python
    expr = 'Scalar(torch.tensor([300.0, 200.0, 100.0, 50.0], dtype=torch.float64), sub_batch_ndim=1)'
  []
  [d_values]
    type = Python
    expr = 'Scalar(torch.tensor([30.0, 20.0, 15.0, 12.0], dtype=torch.float64), sub_batch_ndim=1)'
  []
[]
...
[Models]
  ...
  [implicit_rate]
    ...
    models = 'isoharden elasticity g
              ...
              yield_fn yield_zero normality eprate Eprate Erate Eerate
