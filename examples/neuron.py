...
def driving_current(I_max, R, rate, thold, chold, Ncycles, nload, nhold, neq):
    ...
    for i in range(neq):
        cycle = utility.generate_random_cycle(I_max, R, rate, thold, chold)
        ti, Ii, ci = utility.sample_cycle_normalized_times(cycle, Ncycles, nload, nhold)
...
if __name__ == "__main__":
    ...
    ode_model = HodgkinHuxleyCoupledNeurons(
    ...
    model = ode.BackwardEulerODE(ode_model)

    y0 = torch.rand((nbatch, ode_model.n_equations), device=device)

    ntime = 10
    y_test = y0.unsqueeze(0).expand(ntime, -1, -1)

    R, J = model(y_test, times[:ntime])
    print(R.shape)
    print(J.shape)
