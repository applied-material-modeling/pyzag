...
.. math::
   ...
   \left(x_{i-1}, x_i; p\right) =0 \, \forall i \in \left(1,2,\ldots,n \right)

with :math:`x` the model *state* and :math:`p` the model *parameters*.  Given the model and an *initial condition* :math:`x_o` we can define a sequence :math:`\mathcal{X} = \left(x_0, x_1, \ldots, x_n \right)` by recursively solving the nonlinear equation for :math:`x_n`.
...
.. math::
   ...
   \left(x_{i-1}, x_i; p \right) = x_i - x_{i-1} - g(x_i; p) \Delta t_i
